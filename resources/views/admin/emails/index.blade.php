<x-admin-layout>
    <div class="space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-slate-900">Modelos de E-mail</h1>
                <p class="text-slate-500 mt-1">Gerencie os modelos de e-mail do sistema e marketing.</p>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($templates as $template)
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td class="px-6 py-5">
                                    <p class="text-sm font-medium text-slate-700">{{ $template->subject }}</p>
                                </td>
                                <td class="px-6 py-5">
                                    @if($template->is_system)
                                        <span
                                            class="inline-flex px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide bg-blue-50 text-blue-600">
                                            Sistema
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide bg-amber-50 text-amber-600">
                                            Marketing
                                        </span>
                                    @endif
                                </td>
                                <td class="px-6 py-5 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        @if(!$template->is_system)
                                            <form action="{{ route('admin.emails.destroy', $template) }}" method="POST"
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-20 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <h3 class="text-slate-900 font-bold">Nenhum modelo encontrado</h3>
                                        <p class="text-slate-500 text-sm">Crie seu primeiro modelo de marketing.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
